...
def take_img():
    ...
    detector = cv2.CascadeClassifier('../app/Scripts/haarcascade/haarcascade_frontalface_default.xml')
    ID = txt.get()
    Name = txt2.get()
    sampleNum = 0
    while (True):
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.imwrite("../app/Scripts/dataset/ " + Name + "." + ID + '.' + str(sampleNum) + ".jpg", gray[y:y + h, x:x + w])
...
def trainimg():
    ...
    try:
        global faces,Id
        faces, Id = getImagesAndLabels("../app/Scripts/dataset")
    ...
    try:
        recognizer.save("../app/Scripts/model/trained_model2.yml")
    except Exception as e:
...
txt = tk.Entry(window, validate="key", width=20,  fg="green")
...
txt2 = tk.Entry(window, width=20, fg="green")
